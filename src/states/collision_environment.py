...
class CollisionEnvironment(smach.State):
    ...
    The object is added as a box with the name 'object' and the same dimensions as the bounding box of the object.
    The table is added as a box with the name 'table' and the same dimensions as the bounding box of the table.
    It also adds a floor plane to filter weird octomap points in the floor that prevent the robot from moving because of 'collisions' with the floor.
    ...
    Output keys:
        grasp_object_name_moveit (str): The name of the object in the MoveIt collision environment. 
    ...
    def __init__(self):
        ...
        # This is necessary, because moveit does not really remove the old floor points from the octomap after adding the floor plane. 
    ...
    def execute(self, userdata):
        ...
        # Needed because sometimes the object from the previous grasp is still attached to the eef if MoveIt bugs out (aka we don't handle all possible MoveIt errors properly)
        self.moveit_wrapper.detach_all_objects()
