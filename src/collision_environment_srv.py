...
from grasping_pipeline_msgs.srv import SetCollisionEnvironment, SetCollisionEnvironmentResponse

class CollisionEnvironmentService():
    '''
    ROS service for setting the MoveIt collision environment.
    ...
    def __init__(self):
        ...
        '''
        rospy.init_node('collision_environment_srv')
        ...
        self.service = rospy.Service('set_collision_environment', SetCollisionEnvironment, self.handle_set_collision_environment)

        rospy.loginfo('Collision Environment Service is ready.')
    ...
    def handle_set_collision_environment(self, req):
        '''
        ...
        rospy.loginfo("[CollisionEnvironmentService] Setting collision environment...")
        try:
            self.moveit_wrapper.detach_all_objects()

            grasp_obj_bb = req.grasp_object_bb
            ...
            self.moveit_wrapper.add_box(
                'table', 
                req.table_bbs.header.frame_id, 
                table_bb.center, 
                vector3_to_list(table_bb.size))
            
            res = SetCollisionEnvironmentResponse()
            res.success = True
            res.grasp_object_name_moveit = 'object'
            rospy.loginfo("[CollisionEnvironmentService] Collision environment set.")
        except Exception as e:
            rospy.logerr(f'Failed to set collision environment: {e}')
            res = SetCollisionEnvironmentResponse()
            res.success = False
            res.grasp_object_name_moveit = ''

        return res

if __name__ == "__main__":
    try:
        CollisionEnvironmentService()
        rospy.spin()
